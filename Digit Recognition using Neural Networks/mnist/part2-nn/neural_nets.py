...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        ReLU_der = np.vectorize(rectified_linear_unit_derivative)
        ...
        #print(output_layer_error*output_layer_error/2)
        reluD = ReLU_der(hidden_layer_weighted_input)
        temp = np.multiply(self.hidden_to_output_weights.T, reluD)
        hidden_layer_error = np.multiply(output_layer_error,temp)# TODO (3 by 1 matrix)
        ...
        input_to_hidden_weight_gradients = np.matmul(hidden_layer_error,np.transpose(input_values))# TODO
